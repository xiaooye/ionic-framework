...
test.describe('textarea: item', () => {
  ...
  test('should render correctly in list with solid fill', async ({ page }) => {
    ...
    `);
    const list = page.locator('ion-list');
    expect(await list.screenshot()).toMatchSnapshot(`textarea-list-solid-${page.getSnapshotSettings()}.png`);
  });
  test('should render correctly in list with outline fill', async ({ page }) => {
    ...
    `);
    const list = page.locator('ion-list');
    expect(await list.screenshot()).toMatchSnapshot(`textarea-list-outline-${page.getSnapshotSettings()}.png`);
  });
